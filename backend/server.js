...
const checkLock = async (req, res, next) => {
  ...
  try {
    const [rows] = await pool.execute('SELECT is_locked, locked_by_user_id FROM emails_editable WHERE uuid = ?', [
      uuid,
    ]);
    if (rows.length === 0) {
...
app.delete('/api/emails-editable/:uuid', protect, async (req, res) => {
  ...
  try {
    ...
    const [emailRows] = await pool.execute('SELECT is_locked, locked_by_user_id FROM emails_editable WHERE uuid = ?', [
      uuid,
    ]);
    ...
    if (email.is_locked && email.locked_by_user_id !== currentUserId) {
      ...
      return res
        .status(409)
        .json({ message: `Este correo está siendo editado por ${lockedByUsername} y no se puede eliminar.` });
    }
...
app.post('/api/emails-editable/:uuid/lock', protect, async (req, res) => {
  ...
  try {
    ...
    await pool.execute('UPDATE emails_editable SET is_locked = 1, locked_by_user_id = ? WHERE uuid = ?', [
      currentUserId,
      uuid,
    ]);
...
app.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      await pool.execute('INSERT INTO templates (name, html_content, user_id) VALUES (?, ?, ?)', [
        name,
        html_content,
        user_id,
      ]);
...
app.put(
  ...
  async (req, res) => {
    ...
    try {
      ...
      const [result] = await pool.execute('UPDATE templates SET name = ?, html_content = ? WHERE id = ?', [
        name,
        html_content,
        id,
      ]);
