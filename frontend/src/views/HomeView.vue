<template>
  <div class="card p-4 mt-5 mx-auto" style="max-width: 800px;"> 
    ...
    <div v-if="!loadingTemplates && !templatesError">
      <div v-if="templates.length > 0" class="mb-4"> <label for="selectTemplate" class="form-label d-block">Selecciona un Template:</label>
      ...
      <div v-if="selectedTemplateId && selectedTemplatePlaceholders.length > 0" class="p-4 border rounded bg-light mb-4 text-start"> <h2 class="h5 border-bottom pb-2 mb-3">Contenido Inicial para {{ getTemplateName(selectedTemplateId) }}</h2>
        <div v-for="placeholder in selectedTemplatePlaceholders" :key="placeholder" class="mb-3"> <label :for="`initial-${placeholder}`" class="form-label">{{ capitalizeFirstLetter(placeholder.replace(/_/g, ' ')) }}:</label>
