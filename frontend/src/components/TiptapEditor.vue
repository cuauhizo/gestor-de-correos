  ...
  <div class="tiptap-editor-wrapper">
    <div v-if="editor" class="tiptap-toolbar">
      ...
      <button @click="editor.chain().focus().setParagraph().run()"
        :class="{ 'is-active': editor.isActive('paragraph') }" title="Párrafo">P</button>
      <button @click="editor.chain().focus().toggleHeading({ level: 1 }).run()"
        :class="{ 'is-active': editor.isActive('heading', { level: 1 }) }" title="Encabezado 1">H1</button>
      <button @click="editor.chain().focus().toggleHeading({ level: 2 }).run()"
        :class="{ 'is-active': editor.isActive('heading', { level: 2 }) }" title="Encabezado 2">H2</button>
      <button @click="editor.chain().focus().toggleBulletList().run()"
        :class="{ 'is-active': editor.isActive('bulletList') }" title="Lista con viñetas">UL</button>
      <button @click="editor.chain().focus().toggleOrderedList().run()"
        :class="{ 'is-active': editor.isActive('orderedList') }" title="Lista numerada">OL</button>
      <button @click="editor.chain().focus().toggleCodeBlock().run()"
        :class="{ 'is-active': editor.isActive('codeBlock') }" title="Bloque de código">Code Block</button>
      <button @click="editor.chain().focus().setTextAlign('left').run()"
        :class="{ 'is-active': editor.isActive({ textAlign: 'left' }) }" title="Alinear Izquierda">
        <i-bi-text-left />
      </button>
      <button @click="editor.chain().focus().setTextAlign('center').run()"
        :class="{ 'is-active': editor.isActive({ textAlign: 'center' }) }" title="Centrar">
        <i-bi-text-center />
      </button>
      <button @click="editor.chain().focus().setTextAlign('right').run()"
        :class="{ 'is-active': editor.isActive({ textAlign: 'right' }) }" title="Alinear Derecha">
        <i-bi-text-right />
      </button>
      ...
      <button v-if="editor.isActive('link')" @click="editor.chain().focus().unsetLink().run()"
        title="Quitar enlace">Unlink</button>
...
const props = defineProps({
  modelValue: {
    // Recibe el HTML inicial del padre
    type: String,
    default: '',
  },
  placeholder: {
    // Opcional: para un placeholder visual
    type: String,
    default: '',
  },
  showHtmlToggle: {
  ...
  isHtmlMode: { // Para que el botón sepa si está "activo"
    type: Boolean,
    default: false
  }
})

const emit = defineEmits(['update:modelValue', 'toggle-html']) // Emite los cambios al padre
...
.tiptap-editor-wrapper {
  ...
  border-radius: 4px;
  overflow: hidden;
  /* Para contener la barra de herramientas y el contenido */
...
.tiptap-toolbar button {
  ...
  font-size: 0.9em;
  min-width: unset;
  /* Sobreescribe el estilo del botón global si lo tienes */
  margin: 0;
  /* Elimina márgenes extra si los tienes en botones globales */
  color: #333;
...
.tiptap-toolbar button.is-active {
  ...
  /* Fondo azul claro para botones activos */
  color: #0056b3;
  border-color: #007bff;
}
...
.tiptap-content {
  padding: 10px;
  min-height: 120px;
  /* Altura mínima para el área de edición */
  outline: none;
